...
def configure_stochastic_optimal_control_problem(ocp: OptimalControlProgram, nlp: NonLinearProgram, wM, wS):
    ...
    ConfigureProblem.configure_dynamics_function(
        ocp,
        nlp,
        dyn_func=lambda states, controls, parameters, stochastic_variables, nlp, wM, wS: nlp.dynamics_type.dynamic_function(
            states, controls, parameters, stochastic_variables, nlp, wM, wS, with_gains=False
        ),
        wM=wM,
        wS=wS,
        expand=False,
...
def minimize_uncertainty(controllers: list[PenaltyController], key: str) -> cas.MX:
    ...
    for i, ctrl in enumerate(controllers):
        P_matrix = ctrl.integrated_values["cov"].reshape_to_matrix(
...
def zero_acceleration(
    controller: PenaltyController, wM: np.ndarray, wS: np.ndarray, force_field_magnitude: float
) -> cas.MX:
    dx = stochastic_forward_dynamics(
...
def prepare_socp(
    ...
    for i in range(n_shooting - 1):
        multinode_constraints.add(
            leuven_trapezoidal, nodes_phase=[0, 0], nodes=[i, i + 1], force_field_magnitude=force_field_magnitude
        )
    ...
    dynamics.add(
        ...
        dynamic_function=lambda states, controls, parameters, stochastic_variables, nlp, wM, wS, with_gains: stochastic_forward_dynamics(
            states,
            controls,
            parameters,
    ...
    integrated_value_functions = {
        "cov": lambda nlp, node_index: get_p_mat(
            nlp,
            node_index,
            force_field_magnitude=force_field_magnitude,
...
def main():
    ...
    # Save .mat files
    sio.savemat(
        f"leuvenarm_muscle_driven_socp_{problem_type}_forcefield{force_field_magnitude}.mat",
        {
            "q_sol": q_sol,
            "qdot_sol": qdot_sol,
            "activations_sol": activations_sol,
    ...
    if PLOT_SOL_FLAG:
        ...
        dyn_fun = cas.Function(
            "dyn_fun", [states, controls, parameters, stochastic_variables, wM_sym, wS_sym], [out.dxdt]
        )
