...
def _is_valid_member(cls_name, name):
    return not (_is_descriptor(name) or _is_sunder(name) or _is_dunder(name) or _is_private(cls_name, name))
...
class MetaFcnEnum(type):
    ...
    def __getattribute__(cls, attr):  # hides bases' members at runtime, but not in PyCharm
        ...
        is_valid_member = not (
            _is_descriptor(obj) or _is_dunder(attr) or _is_sunder(attr) or _is_private(classname, attr)
        )
