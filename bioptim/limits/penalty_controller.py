...
class PenaltyController:
    ...
    def q(self) -> OptimizationVariable:
        if "q" in self.states:
            ...
            q_parent_list = OptimizationVariableList(self._nlp.cx, self._nlp.phase_dynamics == PhaseDynamics.SHARED_DURING_THE_PHASE)
            q_parent_list._cx_start = cx_start
            q = OptimizationVariable(name="q",
                                    ...
                                    mapping=BiMapping([i for i in range(self.states["q_roots"].shape + self.states["q_joints"].shape)], [i for i in range(self.states["q_roots"].shape + self.states["q_joints"].shape)]),
    ...
    def qdot(self) -> OptimizationVariable:
        if "qdot" in self.states:
            ...
            qdot_parent_list = OptimizationVariableList(self._nlp.cx,
                                                     self._nlp.phase_dynamics == PhaseDynamics.SHARED_DURING_THE_PHASE)
            qdot_parent_list._cx_start = cx_start
            qdot = OptimizationVariable(name="qdot",
                                     mx=vertcat(self.states["qdot_roots"].mx, self.states["qdot_joints"].mx),
                                     cx_start=cx_start,
                                     index=[i for i in
                                            range(self.states["qdot_roots"].shape + self.states["qdot_joints"].shape)],
                                     mapping=BiMapping([i for i in range(
                                         self.states["qdot_roots"].shape + self.states["qdot_joints"].shape)], [i for i in
                                                                                                          range(
                                                                                                              self.states[
                                                                                                                  "qdot_roots"].shape +
                                                                                                              self.states[
