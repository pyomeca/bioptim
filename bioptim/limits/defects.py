...
class DynamicsDefect:
    # TODO: This class should handle the defects as objectives

    def __init__(
        self,
        defect: DynamicsDefectFcn,
        node: Node = Node.ALL,
        phase: int = -1,
    ):
        """
        ...
        ----------
        defect: DefectFcn
            The chosen dynamics defect constraint
        node: Node
            The intermediate integration nodes to apply the dynamics defect constraint on
        phase: int
            The index of the phase to apply the dynamics defect constraint
        TODO: Generalize with extra_parameters ans custom
        """

        if node not in [Node.START, Node.INTERMEDIATES, Node.END, Node.ALL]:
            raise RuntimeError("The defect node must be either Node.START, Node.INTERMEDIATES, Node.END or Node.ALL")

        if not isinstance(defect, DynamicsDefectFcn):
            raise RuntimeError("The defect must be a DynamicsDefectFcn for now.")
...
class DynamicsDefectList(OptionList):
    """
    A list of DynamicsDefects if more than one is required

    Methods
    -------
    add(self, defect: DynamicsDefectFcn)
        Add a new dynamics defect constraint to the list
    print(self)
        Print the DynamicsDefectList to the console
    """

    def add(self, defect: DynamicsDefectFcn, node: Node = Node.ALL, phase: int = -1):
        """
        Add a new DynamicsDefect to the list
        ...
        if isinstance(defect, DynamicsDefect):
            self.copy(defect)

        else:
            super(DynamicsDefectList, self)._add(option_type=DynamicsDefect, constraint=defect, node=node, phase=phase)
    ...
    def print(self):
        ...
        raise NotImplementedError("Printing DynamicsDefectList is not ready yet")


class DynamicsDefectFunction:
    """
    ...
    class Functions:
        """
        Implementation of all the dynamics defect constraint functions
        """
