...
class Solution:
    ...
    def __perform_integration(
        ...
        for p, (nlp, t_eval) in enumerate(zip(self.ocp.nlp, out._time_vector)):
            ...
            u = self._controls[self.ocp.nlp[p].use_controls_from_phase_idx]["all"]

            if integrator != SolutionIntegrator.OCP:

                out._states[self.ocp.nlp[p].use_states_from_phase_idx]["all"] = solve_ivp_interface(
                ...
                out._states[self.ocp.nlp[p].use_states_from_phase_idx]["all"] = solve_ivp_bioptim_interface(
