...
def generic_get_all_penalties(interface, nlp: NonLinearProgram, penalties):
    ...
    def get_x_and_u_at_idx(_penalty, _idx):
        if _penalty.transition:
            ocp = interface.ocp
            _x = vertcat(ocp.nlp[_penalty.phase_pre_idx].X_scaled[-1], ocp.nlp[_penalty.phase_post_idx].X_scaled[0][:, 0])
            _u = vertcat(ocp.nlp[_penalty.phase_pre_idx].U_scaled[-1], ocp.nlp[_penalty.phase_post_idx].U_scaled[0])
        elif _penalty.multinode_constraint:
            ocp = interface.ocp
            _x = vertcat(
                ocp.nlp[_penalty.phase_first_idx].X_scaled[_penalty.node_idx[0]],
                ocp.nlp[_penalty.phase_second_idx].X_scaled[_penalty.node_idx[1]][:, 0],
            )
            # Make an exception to the fact that U is not available for the last node
            mod_u0 = 1 if _penalty.first_node == Node.END else 0
            mod_u1 = 1 if _penalty.second_node == Node.END else 0
            _u = vertcat(
                ocp.nlp[_penalty.phase_first_idx].U_scaled[_penalty.node_idx[0] - mod_u0],
                ocp.nlp[_penalty.phase_second_idx].U_scaled[_penalty.node_idx[1] - mod_u1],
            )
    ...
    for penalty in penalties:
        ...
        if penalty.multi_thread:
            ...
            for idx in penalty.node_idx:
                ...
                x_scaled = horzcat(x_scaled, x_tp)
                u_scaled = horzcat(u_scaled, u_tp)
            if (
                penalty.derivative or penalty.explicit_derivative or penalty.node[0] == Node.ALL
            ) and nlp.control_type == ControlType.CONSTANT:
                u_scaled = horzcat(u_scaled, u_scaled[:, -1])

            p = reshape(penalty.weighted_function(x_scaled, u_scaled, param, penalty.weight, target, penalty.dt), -1, 1)
            ...
            for idx in penalty.node_idx:
                ...
                if not nlp:
                    ...
                    x_scaled, u_scaled = get_x_and_u_at_idx(penalty, idx)
                p = vertcat(p, penalty.weighted_function(x_scaled, u_scaled, param, penalty.weight, target, penalty.dt))
