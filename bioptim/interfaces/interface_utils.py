    ...
    def get_x_and_u_at_idx(_penalty, _idx, is_unscaled):
        ...
        if _penalty.transition:
            ...
            if is_unscaled:
                ...
                if interface.ocp.nlp[_penalty.nodes_phase[0]].phase_dynamics == PhaseDynamics.SHARED_DURING_THE_PHASE or _penalty.all_nodes_index[0] < len(interface.ocp.nlp[0].U):
                    if (
                        ...
                        or interface.ocp.nlp[_penalty.nodes_phase[1]].phase_dynamics == PhaseDynamics.SHARED_DURING_THE_PHASE
                    ):
                        fake = interface.ocp.cx(
                            interface.ocp.nlp[_penalty.nodes_phase[1]].U[0].shape[0]
                            - interface.ocp.nlp[_penalty.nodes_phase[0]].U[0].shape[0],
                    ...
                    _u_0 = []
                if interface.ocp.nlp[_penalty.nodes_phase[1]].phase_dynamics == PhaseDynamics.SHARED_DURING_THE_PHASE or _penalty.all_nodes_index[1] < len(
                    ...
                    if (
                        ...
                        or interface.ocp.nlp[_penalty.nodes_phase[0]].phase_dynamics == PhaseDynamics.SHARED_DURING_THE_PHASE
                    ):
                        fake = interface.ocp.cx(
                            interface.ocp.nlp[_penalty.nodes_phase[0]].U[0].shape[0]
                            - interface.ocp.nlp[_penalty.nodes_phase[1]].U[0].shape[0],
                ...
                if interface.ocp.nlp[_penalty.nodes_phase[0]].phase_dynamics == PhaseDynamics.SHARED_DURING_THE_PHASE or _penalty.all_nodes_index[0] < len(
            ...
            for i in range(len(_penalty.nodes_phase)):
                ...
                if is_unscaled:
                    ...
                    _u_tp = nlp_i.U[index_i - ui_mode] if nlp.phase_dynamics == PhaseDynamics.SHARED_DURING_THE_PHASE or index_i < len(nlp_i.U) else []
            ...
            if is_unscaled:
                ...
                _u = nlp.U[_idx][:, 0] if nlp.phase_dynamics == PhaseDynamics.SHARED_DURING_THE_PHASE or _idx < len(nlp.U) else []
                _s = nlp.S[_idx]
            else:
                _x = nlp.cx()
                for i in range(nlp.X_scaled[_idx].shape[1]):
                    _x = vertcat(_x, nlp.X_scaled[_idx][:, i])
                _u = nlp.U_scaled[_idx][:, 0] if nlp.phase_dynamics == PhaseDynamics.SHARED_DURING_THE_PHASE or _idx < len(nlp.U_scaled) else []
        ...
        if _penalty.integration_rule == QuadratureRule.APPROXIMATE_TRAPEZOIDAL:
            ...
            if nlp.control_type == ControlType.LINEAR_CONTINUOUS:
                if is_unscaled:
                    u = nlp.U[_idx + 1][:, 0] if nlp.phase_dynamics == PhaseDynamics.SHARED_DURING_THE_PHASE or _idx + 1 < len(nlp.U) else []
                else:
                    u = (
