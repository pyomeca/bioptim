...
def prepare_ocp(
    ...
    if nb_phases == 3:
        ...
        constraints.add(
            Constraint.TIME_CONSTRAINT, instant=Instant.END, min_bound=time_min[1], max_bound=time_max[1], phase=1
        )
        constraints.add(Constraint.ALIGN_MARKERS, instant=Instant.END, first_marker_idx=0, second_marker_idx=2, phase=2)
        constraints.add(
            Constraint.TIME_CONSTRAINT, instant=Instant.END, min_bound=time_min[2], max_bound=time_max[2], phase=2
        )
