...
def prepare_ocp(biorbd_model_path, number_shooting_points, final_time, max_torque, X0, U0, data_to_track=[],
...
if __name__ == "__main__":
    ...
    Y_i = Y_N_[:, :, :N_mhe]
    ocp = prepare_ocp(biorbd_model_path, number_shooting_points=N_mhe - 1, final_time=Tf_mhe, max_torque=T_max,
                      X0=X0, U0=U0, data_to_track=Y_i)
    ...
    for i in range(1, N-N_mhe):
        Y_i = Y_N_[:, :, i:i + N_mhe]
        ocp.modify_objective_function({"type": Objective.Lagrange.MINIMIZE_MARKERS, "weight": 1000, "data_to_track": Y_i}, 0)
        sol = ocp.solve(options_ipopt={'print_level': 0, 'bound_frac': 1e-5, 'bound_push': 1e-5, })
        data_sol = Data.get_data(ocp, sol)
        X0, U0, X_out = warm_start_mhe(data_sol)
        X_est[:, i] = X_out
