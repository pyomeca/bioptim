...
def test_vector_layout_collocation(duplicate_starting_point):
    ...
    np.save(FILE_LOCATION + f"/v_bounds_min_collocation{file_suffix}.npy", v_bounds[0], allow_pickle=False)
    np.save(FILE_LOCATION + "/v_bounds_max_collocation_starting_point.npy", v_bounds[1], allow_pickle=False)
    np.save(
        FILE_LOCATION + "/v_init_collocation_starting_point.npy",
        OptimizationVectorHelper.init_vector(ocp),
        allow_pickle=False,
    )
