...
def test_double_pendulum_torque_driven_IOCP():
    ...
    ocp = ocp_module.prepare_ocp(weights=[0.4, 0.3, 0.3],
                                 coefficients=[1, 1, 1],
    ...
    np.testing.assert_almost_equal(q[:, 0], np.array([-3.14159265,  0.        ]))
    np.testing.assert_almost_equal(q[:, -1], np.array([3.14159265, 0.        ]))
