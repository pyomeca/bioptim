...
def test_muscle_driven_ocp():
    ocp = static_arm.prepare_ocp(str(PROJECT_FOLDER) + "/examples/emg_and_marker_tracking/arm26.bioMod")
    sol = ocp.solve()
    ...
    np.testing.assert_almost_equal(
        mus[:, 0],
        np.array([2.59841012e-01, 1.37678764e-04, 1.17411900e-02, 6.44958524e-04, 6.61972924e-04, 2.80298348e-02]),
    )
    np.testing.assert_almost_equal(
        mus[:, -1], np.array([0.00023481, 0.00375259, 0.00023331, 0.00566022, 0.00565047, 0.0002493]),
    )
