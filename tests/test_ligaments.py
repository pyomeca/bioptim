...
def test_torque_driven_with_ligament(with_ligament, cx, rigidbody_dynamics):
    # Prepare the program
    ...
    NonLinearProgram.add(
        ...
        "dynamics_type",
        Dynamics(
            DynamicsFcn.TORQUE_DRIVEN, rigidbody_dynamics=rigidbody_dynamics, with_ligament=with_ligament
        ),
        False,
    )
...
def test_muscle_driven_with_ligament(with_ligament, rigidbody_dynamics, cx):
    # Prepare the program
    nlp = NonLinearProgram()
...
def test_pendulum_passive_torque(rigidbody_dynamics, with_ligament):
    from bioptim.examples.torque_driven_ocp import pendulum_with_ligament as ocp_module

    bioptim_folder = os.path.dirname(ocp_module.__file__)

    # Define the problem
    biorbd_model_path = bioptim_folder + "/models/pendulum_with_ligament.bioMod"
    final_time = 0.1
    n_shooting = 5
