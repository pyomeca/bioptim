...
def test_align_and_minimize_marker_displacement(ode_solver):
    ...
    np.testing.assert_almost_equal(qdot[:, 0], np.array([5.03822848,   4.6993718, 10., -10.]))
    np.testing.assert_almost_equal(qdot[:, -1], np.array([-4.76267039, -3.46170478, 10.0, 10.0]))
    # initial and final controls
    np.testing.assert_almost_equal(
        tau[:, 0], np.array([-2.62720589e+01,  4.40828815e+00, -5.68180291e-08,  2.61513716e-08])
    )
    np.testing.assert_almost_equal(
        tau[:, -1], np.array([-2.62720590e+01,  4.40828815e+00,  5.68179790e-08,  2.61513677e-08])
    )
...
def test_align_and_minimize_marker_velocity(ode_solver):
    ...
    # initial and final position
    np.testing.assert_almost_equal(
        q[:, 0], np.array([7.18708669e-01, -4.45703930e-01, -3.14159262e+00, 0])
    )
    np.testing.assert_almost_equal(
        q[:, -1], np.array([1.08646846e+00, -3.86731175e-01,  3.14159262e+00, 0])
    )
    # initial and final velocities
