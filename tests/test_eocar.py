...
spec = importlib.util.spec_from_file_location("eocar", str(PROJECT_FOLDER) + "/examples/eocar/eocar.py")
eocar = importlib.util.module_from_spec(spec)
spec.loader.exec_module(eocar)


def test_oecar():
    nlp = eocar.prepare_nlp(biorbd_model_path=str(PROJECT_FOLDER) + "/examples/eocar/eocar.bioMod")
    sol = nlp.solve()
    ...
    for idx in range(nlp.model.nbQ()):
        ...
        q_dot.append(np.array(sol["x"][1 * nlp.model.nbQ() + idx::3 * nlp.model.nbQ()]))
