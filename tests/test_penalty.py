...
def get_penalty_value(ocp, penalty, t, x, u, p):
    ...
    states = ocp.nlp[0].states[0].cx_start if ocp.nlp[0].states[0].cx_start.shape != (0, 0) else ocp.cx(0, 0)   # TODO: [0] to [node_index]
    controls = ocp.nlp[0].controls[0].cx_start if ocp.nlp[0].controls[0].cx_start.shape != (0, 0) else ocp.cx(0, 0) # TODO: [0] to [node_index]
...
def test_penalty_custom(penalty_origin, value):
    def custom(pn, mult):
        my_values = pn.nlp.states[0]["q"].cx * mult # TODO: [0] to [node_index]
        return my_values
...
def test_penalty_custom_with_bounds(value):
    def custom_with_bounds(pn):
        return -10, pn.nlp.states[0]["q"].cx, 10    # TODO: [0] to [node_index]
