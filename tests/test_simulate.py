...
def test_integrate(use_scipy, ode_solver):
    ...
    for i, key in enumerate(sol.states):
        np.testing.assert_almost_equal(sol_integrated.states[key][:, [0, -1]], sol.states[key][:, [0, -1]], decimal=decimal)
...
def test_integrate_non_continuous(shooting, merge, use_scipy, ode_solver):
    ...
    opts = {"shooting_type": shooting, "continuous": False, "keep_intermediate_points": False, "use_scipy_integrator": use_scipy}
    ...
    for i, key in enumerate(sol.states):
        np.testing.assert_almost_equal(sol_integrated.states[key][:, [0, -1]], sol.states[key][:, [0, -1]], decimal=decimal)
        np.testing.assert_almost_equal(sol_integrated.states[key][:, [0, -2]], sol.states[key][:, [0, -1]], decimal=decimal)
...
def test_integrate_multiphase(shooting, keep_intermediate_points, use_scipy, ode_solver):
    ...
    opts = {"shooting_type": shooting, "continuous": False, "keep_intermediate_points": keep_intermediate_points, "use_scipy_integrator": use_scipy}
    if ode_solver == OdeSolver.COLLOCATION and not use_scipy:
        if shooting != Shooting.MULTIPLE:
    ...
    if ode_solver == OdeSolver.COLLOCATION and not use_scipy:
        with pytest.raises(
            RuntimeError, match="Integration with direct collocation must be not continuous",
        ):
            sol.integrate(**opts)
        return
    ...
    for i in range(len(sol_integrated.states)):
        for k, key in enumerate(sol.states[i]):
            if not use_scipy or shooting == Shooting.MULTIPLE:
                np.testing.assert_almost_equal(sol_integrated.states[i][key][:, [0, -1]], sol.states[i][key][:, [0, -1]], decimal=decimal)
...
def test_integrate_multiphase_non_continuous(shooting, use_scipy, ode_solver):
    ...
    opts = {"shooting_type": shooting, "continuous": False, "keep_intermediate_points": True, "use_scipy_integrator": use_scipy}
    if ode_solver == OdeSolver.COLLOCATION and not use_scipy:
        if shooting != Shooting.MULTIPLE:
            with pytest.raises(RuntimeError, match="Integration with direct collocation must using shooting_type=Shooting.MULTIPLE"):
                _ = sol.integrate(**opts)
    ...
    for i in range(len(sol_integrated.states)):
        for k, key in enumerate(sol.states[i]):
            if not use_scipy or shooting == Shooting.MULTIPLE:
                np.testing.assert_almost_equal(sol_integrated.states[i][key][:, [0, -1]], sol.states[i][key][:, [0, -1]], decimal=decimal)
                np.testing.assert_almost_equal(sol_integrated.states[i][key][:, [0, -2]], sol.states[i][key][:, [0, -1]], decimal=decimal)
