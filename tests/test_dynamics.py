...
def test_torque_derivative_driven(with_contact, with_external_force, cx):
    ...
    ocp = OptimalControlProgram(nlp)
    nlp.control_type = ControlType.CONSTANT
    NonLinearProgram.add(ocp, "dynamics_type", Dynamics(DynamicsFcn.TORQUE_DERIVATIVE_DRIVEN, with_contact=with_contact), False)
    ...
    if with_contact:
        ...
        if with_external_force:
            np.testing.assert_almost_equal(
                x_out[:, 0],
                [0.86310343,   0.32518332,   0.11959425,   0.4937956 ,
                -7.8965868 ,  -7.70152137,  22.06077635, -16.65932927,
                 0.80744016,   0.42710779,   0.417411  ,   0.32320293],
            )
            np.testing.assert_almost_equal(contact_out[:, 0], [-48.4505407, 111.4024916, -24.4449121])
        else:
            np.testing.assert_almost_equal(
                x_out[:, 0], [0.61185289,  0.78517596,  0.60754485,  0.80839735, -0.32149054,
                   -0.19121314,  0.65071636, -0.23597164,  0.38867729,  0.54269608,
                    0.77224477,  0.72900717]
            )
            np.testing.assert_almost_equal(contact_out[:, 0], [-2.444071, 128.8816865, 2.7245124])
        ...
        if with_external_force:
            np.testing.assert_almost_equal(
                x_out[:, 0],
                [0.86310343,  0.32518332,  0.11959425,  0.4937956 ,  0.30731739,
                   -9.97912778,  1.15263778, 36.02430956,  0.80744016,  0.42710779,
                    0.417411  ,  0.32320293],
            )
        else:
            np.testing.assert_almost_equal(
...
def test_muscle_driven(with_excitations, with_contact, with_residual_torque, with_external_force, cx):
    ...
    ocp = OptimalControlProgram(nlp)
    nlp.control_type = ControlType.CONSTANT
    NonLinearProgram.add(ocp, "dynamics_type", Dynamics(DynamicsFcn.MUSCLE_DRIVEN, with_residual_torque=with_residual_torque, with_excitations=with_excitations, with_contact=with_contact), False)
    ...
    if with_residual_torque:
        if with_excitations:
            if with_external_force:
                np.testing.assert_almost_equal(
                    x_out[:, 0],
                    [0.6158501 ,   0.50313626,   0.64241928,   0.3264777 ,
                    -1.57134516,   0.87073117,  46.87928022,  -1.80189035,
                    53.3914525 ,  48.30056919,  63.69373374, -28.15700995],
                )
            else:
                np.testing.assert_almost_equal(
                    x_out[:, 0],
                    [1.83404510e-01,  6.11852895e-01,  7.85175961e-01,  3.92710810e-02,
                        2.24914101e+00, -9.32712397e+00,  8.60630831e+00,  3.19433638e+00,
                        2.97405608e+01, -2.02754226e+01, -2.32467778e+01, -4.19135012e+01],
                )
        else:
            if with_external_force:
                np.testing.assert_almost_equal(
        ...
        if with_excitations:
            if with_external_force:
                np.testing.assert_almost_equal(
                    x_out[:, 0],
                    [6.15850098e-01,  5.03136259e-01,  6.42419278e-01,  3.91853634e-02,
           -1.76074913e+00,  1.02811024e+00,  5.56555782e+01,  5.04705269e+01,
            3.60255887e-01,  5.89237749e+01,  2.97009419e+01, -1.51353494e+01],
                )
            else:
                np.testing.assert_almost_equal(
                    x_out[:, 0],
                    [1.83404510e-01,  6.11852895e-01,  7.85175961e-01, -7.74768714e-02,
            2.30892158e+00, -9.64013318e+00, -7.72228930e+00, -1.13759732e+01,
            9.51906209e+01,  4.45077128e+00, -5.20261014e+00, -2.80864106e+01],
                )
        else:
            if with_external_force:
                np.testing.assert_almost_equal(
