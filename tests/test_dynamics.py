        ...
        if with_contact:
            ...
            if with_external_force:
                if with_passive_torque:
                    ...
                    np.testing.assert_almost_equal(
                        x_out[:, 0],
                        [0.8631034, 0.3251833, 0.1195942, 0.4937956, -9.3613118, - 9.1282122, 26.1502096, -20.7487625],
                    )
                ...
                if with_passive_torque:
                    ...
                    np.testing.assert_almost_equal(
                        x_out[:, 0],
                        [0.6118529, 0.785176, 0.6075449, 0.8083973, -2.5432261, - 0.2369529, 5.095129, -4.6803842],
                    )
            ...
            if with_external_force:
                if with_passive_torque:
                    ...
                    np.testing.assert_almost_equal(
                        x_out[:, 0],
                        [0.8631034, 0.3251833, 0.1195942, 0.4937956, -3.2691251, - 13.7943226, 20.6084125, 39.1295764],
                    )
                else:
                    np.testing.assert_almost_equal(
                        x_out[:, 0],
    ...
    if with_contact:
        ...
        if with_external_force:
            if with_passive_torque:
                ...
                np.testing.assert_almost_equal(
                    x_out[:, 0],
                    [0.8631, 0.32518, 0.11959, 0.4938, 17.42757, 16.96505, - 48.64291, 54.04436],
                    decimal=5,
                )
            ...
            if with_passive_torque:
                ...
                np.testing.assert_almost_equal(
                    x_out[:, 0],
                    [0.61185, 0.78518, 0.60754, 0.8084, -1.43718, -0.21418, 2.88257, -2.46783]
                    ,
                    decimal=5,
                )
...
def test_muscle_driven(with_excitations, with_contact, with_torque, with_external_force, with_passive_torque, rigidbody_dynamics, cx):
    # Prepare the program
    nlp = NonLinearProgram()
    nlp.model = biorbd.Model(
