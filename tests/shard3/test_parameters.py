...
def test_param_scaling():
    ...
    with pytest.raises(
        ...
        param.add("gravity_z", function=my_parameter_function, size=1, scaling=VariableScaling("gravity_z", np.array([[[1]]])))

    with pytest.raises(ValueError, match=f"Parameter scaling must have exactly one column"):
        param.add("gravity_z", function=my_parameter_function, size=3, scaling=VariableScaling("gravity_z", np.array([1, 2])))

    with pytest.raises(ValueError, match=f"Parameter scaling must have exactly one column"):
        param.add("gravity_z", function=my_parameter_function, size=3, scaling=VariableScaling("gravity_z", np.ones((3, 2))))
