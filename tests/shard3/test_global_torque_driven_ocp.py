...
def test_example_quaternions(phase_dynamics):
    ...
    np.testing.assert_almost_equal(
        qdot_joints[:, 0], np.array([-0.09999998, -0.09999997, 0.09988598, -0.09999997, -0.09999998, 0.099921371])
    )
    np.testing.assert_almost_equal(
        q_roots[:, -1],
        np.array(
    ...
    np.testing.assert_almost_equal(
        tau_joints[:, 0],
        np.array([-0.0427604, -0.03458199, -0.00089253, -0.03687493, -0.04412906, -0.00065701]),
        decimal=6,
    )
    np.testing.assert_almost_equal(
        tau_joints[:, -2],
        np.array([-2.18024140e-03, -3.79700405e-03, 9.31965278e-05, -2.21236997e-03, -4.45420895e-03, -4.93034005e-05]),
        decimal=6,
    )
