...
def test_arm_reaching_torque_driven_collocations():
    ...
    f = np.array(sol.cost)
    np.testing.assert_equal(f.shape, (1, 1))
    np.testing.assert_almost_equal(f[0, 0], 7066.79267067254)

    # detailed cost values
    np.testing.assert_almost_equal(sol.detailed_cost[0]["cost_value_weighted"], 78.47204822049429)
    np.testing.assert_almost_equal(sol.detailed_cost[1]["cost_value_weighted"], 6988.320622452046)
    np.testing.assert_almost_equal(
        f[0, 0], sum(sol.detailed_cost[i]["cost_value_weighted"] for i in range(len(sol.detailed_cost)))
    )
    ...
    np.testing.assert_almost_equal(
        ...
        np.array(
            [
                -0.03780896, 0.01409505, 0.01266273, 0.002933, 0.01428981,
                -0.03905632, 0.01516653, 0.0052785, 0.01329116, 0.01315758,
                -0.03945049, -0.00021098, 0.0136845, 0.01010664, 0.01487932,
                -0.02575473, -0.13110697, 0.0132242, 0.01021474, -0.03393887,
                0.01511638, -0.13156635, 0.01886193, -0.03565643, 0.01233682,
                0.01041309, -0.13657921, -0.02612223, 0.01759883, 0.0114354,
                0.01491626, -0.11649588, -0.12401404, 0.00522018, 0.004885,
                -0.00968069, 0.00574906, -0.12361953, 0.00727845, -0.01105033,
                0.00545047, 0.00471038, -0.12477003, -0.0044019, 0.00770349,
                0.00646891, 0.00625501, -0.12087302,
            ]
        ),
    )
