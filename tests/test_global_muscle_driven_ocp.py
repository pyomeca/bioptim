...
def test_muscle_driven_ocp(ode_solver):
    ...
    if ode_solver == OdeSolver.IRK:
        ...
        np.testing.assert_almost_equal(
            obj[0],
            np.array(
                [[5.86130950e-06, 4.90858403e-06, 4.11439824e-06, 3.23202480e-06,
                 2.31951020e-06, 1.53065911e-06, 9.51069088e-07, 5.96999314e-07,
                 4.49941576e-07, 1.11561148e-07],
                [9.60779353e-03, 7.74754706e-03, 5.59910323e-03, 2.97564968e-03,
                 1.03602703e-03, 2.17877317e-04, 2.73703489e-05, 1.21820523e-05,
                 1.78251397e-05, 5.90838337e-06],
                [np.nan, np.nan, np.nan, np.nan,
                 np.nan, np.nan, np.nan, np.nan,
                 np.nan, 1.16237781e-01]]
            ),
        )
...
def test_muscle_excitation_with_contact_driven_ocp(ode_solver):
    ...
    if ode_solver == OdeSolver.IRK:
        ...
        np.testing.assert_almost_equal(qdot[:, 0], np.array([0.0, 0.0, 0.0, 0.0]))
        np.testing.assert_almost_equal(
            qdot[:, -1], np.array([-2.01866580e+00,  4.49415846e-04,  4.03733171e+00, -4.03733171e+00])
        )
        # initial and final muscle state
        np.testing.assert_almost_equal(mus_states[:, 0], np.array([0.5]))
