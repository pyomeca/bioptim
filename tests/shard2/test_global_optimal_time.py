...
def test_pendulum_min_time_lagrange(ode_solver, phase_dynamics):
    ...
    if ode_solver == OdeSolver.IRK:
        ...
        np.testing.assert_almost_equal(tau[:, -1], np.array((-68.84256311, 0)))
...
def test_monophase_time_constraint(ode_solver, phase_dynamics):
    ...
    if ode_solver == OdeSolver.COLLOCATION:
        ...
        np.testing.assert_almost_equal(g, np.concatenate((np.zeros((120 * 5, 1)), np.array([[0, 0, 0, 0, 0, 0, 1]]).T)), decimal=6)
    else:
