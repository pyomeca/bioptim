...
def test_pendulum(control_type, integration_rule, objective, phase_dynamics):
    ...
    if integration_rule == QuadratureRule.RECTANGLE_LEFT:
        if control_type == ControlType.CONSTANT:
            if objective == "torque":
                TestUtils.assert_objective_value(sol=sol, expected_value=36.077211633874164)
                npt.assert_almost_equal(tau[:, -1], np.array([-15.79894366, 0.0]))
            else:
                TestUtils.assert_objective_value(sol=sol, expected_value=18.91863487850207)
            ...
            if objective == "torque":
                TestUtils.assert_objective_value(sol=sol, expected_value=36.077211633874185)
                ...
                TestUtils.assert_objective_value(sol=sol, expected_value=18.918634878502065)

        elif control_type == ControlType.LINEAR_CONTINUOUS:
            if objective == "torque":
                TestUtils.assert_objective_value(sol=sol, expected_value=52.0209218166193)
            else:
                TestUtils.assert_objective_value(sol=sol, expected_value=18.844221574687065)
        ...
        if control_type == ControlType.CONSTANT:
            if objective == "torque":
                ...
                npt.assert_almost_equal(tau[:, -1], np.array([-17.24468626, 0.0]))
                TestUtils.assert_objective_value(sol=sol, expected_value=18.91863487850206)
        elif control_type == ControlType.CONSTANT_WITH_LAST_NODE:
            npt.assert_equal(np.isnan(tau[:, -1]), np.array([False, False]))
            if objective == "torque":
                TestUtils.assert_objective_value(sol=sol, expected_value=36.077211633874164)
            ...
            if objective == "torque":
                TestUtils.assert_objective_value(sol=sol, expected_value=52.0209218166202)
            else:
                TestUtils.assert_objective_value(sol=sol, expected_value=18.844221574687094)
        ...
        if control_type == ControlType.CONSTANT:
            if objective == "torque":
                ...
                TestUtils.assert_objective_value(sol=sol, expected_value=18.112963129413707)
        elif control_type == ControlType.CONSTANT_WITH_LAST_NODE:
            npt.assert_equal(np.isnan(tau[:, -1]), np.array([0, 0]))
            if objective == "torque":
                TestUtils.assert_objective_value(sol=sol, expected_value=36.077211633874384)
            else:
                TestUtils.assert_objective_value(sol=sol, expected_value=17.944878542423062)
        elif control_type == ControlType.LINEAR_CONTINUOUS:
            if objective == "torque":
                TestUtils.assert_objective_value(sol=sol, expected_value=34.52084504124038)
            else:
                TestUtils.assert_objective_value(sol=sol, expected_value=17.72098984554101)
...
def test_pendulum_target(control_type, integration_rule, objective, phase_dynamics):
    ...
    if integration_rule == QuadratureRule.RECTANGLE_LEFT:
        if control_type == ControlType.CONSTANT:
            if objective == "torque":
                TestUtils.assert_objective_value(sol=sol, expected_value=47.409664872029175)
            else:
                TestUtils.assert_objective_value(sol=sol, expected_value=69.3969483839429, decimal=4)
        elif control_type == ControlType.CONSTANT_WITH_LAST_NODE:
            if objective == "torque":
                TestUtils.assert_objective_value(sol=sol, expected_value=47.409664872029175)
            else:
                TestUtils.assert_objective_value(sol=sol, expected_value=69.39694769390407, decimal=4)
        elif control_type == ControlType.LINEAR_CONTINUOUS:
            if objective == "torque":
                TestUtils.assert_objective_value(sol=sol, expected_value=47.18288247657242)
            else:
                TestUtils.assert_objective_value(sol=sol, expected_value=49.52081908930845)
    elif integration_rule == QuadratureRule.APPROXIMATE_TRAPEZOIDAL:
        if control_type == ControlType.CONSTANT:
            if objective == "torque":
                TestUtils.assert_objective_value(sol=sol, expected_value=49.30669186418164)
            else:
                TestUtils.assert_objective_value(sol=sol, expected_value=69.39694990777029, decimal=5)
        elif control_type == ControlType.CONSTANT_WITH_LAST_NODE:
            if objective == "torque":
                TestUtils.assert_objective_value(sol=sol, expected_value=49.30669186418164)
            else:
            ...
            if objective == "torque":
                TestUtils.assert_objective_value(sol=sol, expected_value=50.204686376505116)
            else:
                TestUtils.assert_objective_value(sol=sol, expected_value=49.52081908924932)
    elif integration_rule == QuadratureRule.TRAPEZOIDAL:
        if control_type == ControlType.CONSTANT:
            if objective == "torque":
                TestUtils.assert_objective_value(sol=sol, expected_value=49.30669186418164)
            else:
                TestUtils.assert_objective_value(sol=sol, expected_value=35.741353753106395)
        if control_type == ControlType.CONSTANT_WITH_LAST_NODE:
            if objective == "torque":
                TestUtils.assert_objective_value(sol=sol, expected_value=49.30669186418164)
            else:
                TestUtils.assert_objective_value(sol=sol, expected_value=35.837674550552975)
        elif control_type == ControlType.LINEAR_CONTINUOUS:
            if objective == "torque":
                TestUtils.assert_objective_value(sol=sol, expected_value=46.04924263465744)
            else:
                TestUtils.assert_objective_value(sol=sol, expected_value=36.05789359834472)
