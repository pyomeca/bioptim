...
def test_track_vector_orientation(phase_dynamics):
    ...
    f = np.array(sol.cost)
    npt.assert_equal(f.shape, (1, 1))
    npt.assert_almost_equal(f[0, 0], 2.614556858357712e-08)
    ...
    # initial and final position
    npt.assert_almost_equal(q[:, 0], np.array([0.80000001, -0.68299837, -1.57, -1.56999089]))
    npt.assert_almost_equal(q[:, -1], np.array([0.80000001, -0.68299837, 1.57, 1.56999138]))
    # initial and final velocities
    npt.assert_almost_equal(qdot[:, 0], np.array((3.37753150e-16, 4.90499995e00, 3.14001172e00, 3.13997056e00)))
    npt.assert_almost_equal(qdot[:, -1], np.array((3.37753131e-16, -4.90499995e00, 3.14001059e00, 3.13997170e00)))
    # initial and final controls
    npt.assert_almost_equal(tau[:, 0], np.array([-1.27269674e-24, 1.97261036e-08, -3.01420965e-05, 3.01164220e-05]))
    npt.assert_almost_equal(tau[:, -1], np.array([-2.10041085e-23, 1.97261036e-08, 2.86303889e-05, -2.86047182e-05]))
