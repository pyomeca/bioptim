...
@pytest.mark.parametrize("defects_type", [DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS, DefectType.TAU_EQUALS_INVERSE_DYNAMICS])
        ...
        if ContactType.RIGID_EXPLICIT in contact_types and defects_type == DefectType.TAU_EQUALS_INVERSE_DYNAMICS:
            ...
            x_defects = np.array(
                nlp.dynamics_defects_func(
                    time, states[:, 0], controls[:, 0], params[:, 0], algebraic_states[:, 0], numerical_timeseries, states_dot[:, 0]
                )
            )
            if ContactType.RIGID_EXPLICIT in contact_types:
                if with_external_force:
                    if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                        npt.assert_almost_equal(x_defects[:, 0],
                                                np.array([-0.04383072, -0.04283728,  0.10537375, -0.04831128,  0.52484935,
                                                         0.18769219, -1.00117517,  0.87117926]))
                else:
                    if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                        npt.assert_almost_equal(x_defects[:, 0],
                                                np.array([-0.31172315, -0.07805808,  0.23040588,  0.07221787,  0.17309483,
                                                    0.4613264 ,  0.63518686,  0.05627764]))
            elif ContactType.RIGID_IMPLICIT in contact_types:
                if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                    npt.assert_almost_equal(x_defects[:, 0],
                                            np.array([ -0.39365036,  -0.09857334,   0.29096126,   0.09119821,
                                            -0.1654925 ,   8.56753594,   1.90034696, -15.91347941,
                                             0.77091916,   0.73515993,   1.10755289])
                                            )
                else:
                    npt.assert_almost_equal(x_defects[:, 0],
                                            np.array([-3.93650365e-01, -9.85733354e-02,  2.90961259e-01,  9.11982054e-02,
                                           -6.86116772e+00, -1.41889201e+02, -4.93022460e+00, -1.49950526e+00,
                                            7.70919162e-01,  7.35159931e-01,  1.10755289e+00]), decimal=6)
            else:
                if with_external_force:
                    if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                        npt.assert_almost_equal(x_defects[:, 0],
                                                np.array([-0.04383072, -0.04283728,  0.10537375, -0.04831128,  0.28869817,
                                                    2.36916856, -1.15701705, -3.82699645]))
                    else:
                        npt.assert_almost_equal(x_defects[:, 0],
                                                np.array([-4.38307239e-02, -4.28372820e-02,  1.05373750e-01, -4.83112831e-02,
                                                -1.07342160e+00, -1.39608973e+02, -5.36386140e+00, -9.73899501e-01]), decimal=6)
                else:
                    if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                        npt.assert_almost_equal(x_defects[:, 0],
                                                np.array([ -0.31172315,  -0.07805808,   0.23040588,   0.07221787,
                                                        -0.13508945,   6.797133  ,   1.523731  , -12.70223002])
                                                )
                    else:
                        npt.assert_almost_equal(x_defects[:, 0],
                                                np.array([-3.11723149e-01, -7.80580770e-02,  2.30405883e-01,  7.22178723e-02,
                                                -6.89259691e+00, -1.42138493e+02, -4.94850146e+00, -1.49950526e+00]), decimal=6)
...
def test_torque_driven_soft_contacts_dynamics(contact_types, cx, phase_dynamics, defects_type):
    ...
    if ContactType.SOFT_IMPLICIT in contact_types:
        if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
            npt.assert_almost_equal(x_defects[:, 0],
                                    np.array([-3.09712665e-01, -7.75546351e-02,  2.28919861e-01,  7.17520972e-02,
                    2.61640603e-02,  7.03511775e+00,  6.83158357e-01, -3.28035543e+01,
                   -3.14291857e-02, -2.49292229e-01, -2.89751453e-01, -8.71460590e-01,
                   -8.07440155e-01, -4.27107789e-01, -4.17411003e-01, -3.23202932e-01,
                   -9.62447295e-01, -3.68869474e-02, -9.08265886e-01, -2.42055272e-01]))
        else:
            npt.assert_almost_equal(x_defects[:, 0],
                                    np.array([-3.09712665e-01, -7.75546351e-02,  2.28919861e-01,  7.17520972e-02,
               -6.64330468e+00, -1.41848741e+02, -4.01919389e+00, -6.97799376e-01,
               -3.14291857e-02, -2.49292229e-01, -2.89751453e-01, -8.71460590e-01,
               -8.07440155e-01, -4.27107789e-01, -4.17411003e-01, -3.23202932e-01,
               -9.62447295e-01, -3.68869474e-02, -9.08265886e-01, -2.42055272e-01]), decimal=6)
    if ContactType.SOFT_EXPLICIT in contact_types:
        if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
            ...
            npt.assert_almost_equal(x_defects[:, 0],
                                    np.array([-3.11723149e-01, -7.80580770e-02,  2.30405883e-01,  7.22178723e-02,
                -6.89259691e+00, -1.42138493e+02, -4.94850146e+00, -1.49950526e+00]),
                                    decimal=6)
...
def test_torque_derivative_driven(with_contact, with_external_force, cx, phase_dynamics, defects_type):
    ...
    if with_contact and defects_type == DefectType.TAU_EQUALS_INVERSE_DYNAMICS:
        with pytest.raises(NotImplementedError,
                           match="Inverse dynamics, cannot be used with ContactType.RIGID_EXPLICIT yet"):
            ConfigureProblem.initialize(ocp, nlp)
        ...
        if with_contact:
            ...
            if with_external_force:
                if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                    ...
                    npt.assert_almost_equal(x_defects[:, 0],
                                            np.array([-4.38307239e-02, -4.28372820e-02, 1.05373750e-01, -4.83112831e-02,
                                                      -1.07342160e+00, -1.39608973e+02, -5.36386140e+00, -9.73899501e-01]),
                                            decimal=6)
            else:
                if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                    npt.assert_almost_equal(x_defects[:, 0],
...
def test_torque_derivative_driven_soft_contacts_dynamics(contact_types, cx, phase_dynamics, defects_type):
    ...
    if ContactType.SOFT_IMPLICIT in contact_types:
        if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
            npt.assert_almost_equal(x_defects[:, 0],
                                    np.array([-0.01877799, -0.02040225,  0.01385785, -0.00667989,  0.10275146,
                                                0.90393653, -0.14864133, -3.54475834, -0.06529354, -0.01496139,
                                               -0.01074127,  0.04613038, -0.96244729, -0.03688695, -0.90826589,
                                               -0.24205527, -0.36778313, -0.8353025 , -0.67756436, -0.17436643,
                                               -0.34106635, -0.65998405, -0.09310277, -0.34920957]))
        else:
            npt.assert_almost_equal(x_defects[:, 0],
                                    np.array([-1.87779876e-02, -2.04022485e-02,  1.38578453e-02, -6.67988697e-03,
                                       -1.16039573e+01, -1.37716732e+02, -4.76824404e+00, -8.16422811e-01,
                                       -6.52935358e-02, -1.49613950e-02, -1.07412701e-02,  4.61303802e-02,
                                       -9.62447295e-01, -3.68869474e-02, -9.08265886e-01, -2.42055272e-01,
                                       -3.67783133e-01, -8.35302496e-01, -6.77564362e-01, -1.74366429e-01,
                                       -3.41066351e-01, -6.59984046e-01, -9.31027678e-02, -3.49209575e-01]), decimal=6)
    if ContactType.SOFT_EXPLICIT in contact_types:
        if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
            npt.assert_almost_equal(x_defects[:, 0],
                                    np.array([-0.05619167, -0.06105214,  0.04146852, -0.01998904,  0.29345657,
                                            2.69664409, -0.37386083, -8.89007806, -0.19538582, -0.04477081,
                                           -0.03214241,  0.13804157]))
        else:
            npt.assert_almost_equal(x_defects[:, 0],
                                    np.array([-5.61916662e-02, -6.10521404e-02,  4.14685233e-02, -1.99890418e-02,
                                           -1.16408442e+01, -1.38624998e+02, -4.97910457e+00, -9.99496342e-01,
                                           -1.95385823e-01, -4.47708098e-02, -3.21424146e-02,  1.38041572e-01]),
                                    decimal=6)
    ...
    if defects_type == DefectType.TAU_EQUALS_INVERSE_DYNAMICS:
        ...
        if with_contact:
            if with_external_force:
                if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                    npt.assert_almost_equal(x_defects[:, 0],
                                            np.array([-0.04383072, -0.04283728,  0.10537375, -0.04831128,  2.74968269,
                                                    0.46056293, -5.48418391,  5.354188  ]),
                                            decimal=5)
            else:
                if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                    npt.assert_almost_equal(x_defects[:, 0],
                                            np.array([ -0.31172315,  -0.07805808,   0.23040588,   0.07221787,
                                                    -28.13827671,  -0.1215312 ,  57.2699282 , -56.5784637 ]),
                                            decimal=5)
            ...
            if with_external_force:
                if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                    npt.assert_almost_equal(x_defects[:, 0],
                                            np.array([-4.38307239e-02, -4.28372820e-02,  1.05373750e-01, -4.83112831e-02,
                                                        1.61331714e+01,  8.13653930e+00, -7.65761291e+01, -3.89924217e+02]),
                                            decimal=5)
            else:
                if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                    npt.assert_almost_equal(x_defects[:, 0],
                                            np.array([-3.11723149e-01, -7.80580770e-02,  2.30405883e-01,  7.22178723e-02,
                                                    -4.20800305e+01,  2.16475239e+01,  1.24723562e+02, -1.37029516e+03]),
                                            decimal=5)
    ...
    if defects_type == DefectType.TAU_EQUALS_INVERSE_DYNAMICS:
        ...
        if with_residual_torque:
            if with_passive_torque:
                if with_external_force:
                    npt.assert_almost_equal(x_defects[:, 0],
                                            np.array([ 6.04508047e-01,  1.84981427e-01, -8.70762306e+01, -1.03464970e+03]),
                                            decimal=5)
                else:
                    npt.assert_almost_equal(x_defects[:, 0],
                                            np.array([  0.16044011,   0.1632901 ,  -1.06310829, -27.22644766]),
                                            decimal=5)
            else:
                if with_external_force:
                    npt.assert_almost_equal(x_defects[:, 0],
                                            np.array([ 6.04508047e-01,  1.84981427e-01, -8.35362294e+01, -1.00272333e+03]),
                                            decimal=5)
                else:
                    npt.assert_almost_equal(x_defects[:, 0],
                                            np.array([  0.16044011,   0.1632901 ,  -0.31706506, -17.5258886 ]),
                                            decimal=5)
        else:
            if with_passive_torque:
                if with_external_force:
                    npt.assert_almost_equal(x_defects[:, 0],
                                            np.array([ 5.28274079e-01,  1.61653585e-01, -7.53323145e+01, -8.95479455e+02]),
                                                decimal=5)
                else:
                    npt.assert_almost_equal(x_defects[:, 0],
                                            np.array([  0.45831154,   0.4664528 ,  -2.70311395, -51.49785706]),
                                            decimal=5)
            else:
                if with_external_force:
                    npt.assert_almost_equal(x_defects[:, 0],
                                            np.array([ 5.28274079e-01,  1.61653585e-01, -7.22387397e+01, -8.67579288e+02]),
                                            decimal=5)
                else:
                    npt.assert_almost_equal(x_defects[:, 0],
                                            np.array([  0.45831154,   0.4664528 ,  -0.57197461, -23.78734089]),
                                            decimal=5)
...
def test_torque_driven_free_floating_base(cx, phase_dynamics, defects_type):
    ...
    x_defects = np.array(
        nlp.dynamics_defects_func(
            time, states[:, 0], controls[:, 0], params[:, 0], algebraic_states[:, 0], [],
            states_dot[:, 0]
        )
    )
    ...
    if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
        npt.assert_almost_equal(x_defects[:, 0],
                                np.array([ -0.3573718 ,  -0.08948888,   0.26414646,   0.08279344,
                                        -0.25072741,   7.9598835 ,   2.1430355 , -25.96856935]),
                                decimal=5)
    else:
        npt.assert_almost_equal(x_defects[:, 0],
                                np.array([-0.3573718 , -0.08948888,  0.26414646,  0.08279344, -1.99330085,
                                                -1.99330085, -1.99330085, -1.13019743]),
                                decimal=6)
...
@pytest.mark.parametrize("defects_type", [DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS, DefectType.TAU_EQUALS_INVERSE_DYNAMICS])
def test_muscle_driven(with_excitations, contact_types, with_residual_torque, with_external_force, cx, phase_dynamics, defects_type):
            ...
            if ContactType.RIGID_EXPLICIT in contact_types:
                if with_residual_torque:
                    if with_excitations:
                        if with_external_force:
                            npt.assert_almost_equal(x_defects[:, 0],
                                                    np.array([ -0.39414169,  -0.34540175,  -0.64642364,  -0.32407953,
                                                            28.31157446,  11.05514434, -29.35153822,  -7.14085992,
                                                            18.29768098,  11.45405575,   7.31449586,   4.18729136]),
                                                    decimal=5)
                        else:
                            npt.assert_almost_equal(x_defects[:, 0],
                                                    np.array([ 6.58474157e-02, -2.21841265e-02, -4.14806864e-03,  1.51595699e+00,
                                           -4.10616583e+01,  1.46386667e+02,  3.25656270e+00, -2.72705314e+00,
                                            4.76521837e-01, -5.19721258e+00,  1.14956080e+01,  8.64588438e+00]),
                                                    decimal=5)
                    else:
                        if with_external_force:
                            npt.assert_almost_equal(x_defects[:, 0],
                                                    np.array([ -0.14206822,  -0.22146577,  -0.07762636,  -0.18970781,
                                                    31.78037536, -23.11015379]),
                                                    decimal=5)
                        else:
                            npt.assert_almost_equal(x_defects[:, 0],
                                                    np.array([ 0.17405995,  0.08065849, -0.2721404 ,  0.46729892, -1.66989605,
                                        49.55158537]),
                                                    decimal=5)
                else:
                    if with_excitations:
                        if with_external_force:
                            npt.assert_almost_equal(x_defects[:, 0],
                                                    np.array([-6.16079563e-02, -5.39894568e-02, -1.01041935e-01, -2.17790397e-03,
                                                        3.85980483e+00,  3.53803553e+00, -5.35239171e+00,  1.16917330e+00,
                                                        1.95016256e+00, -7.20251522e-01,  1.05381874e+00,  4.26798401e-01]),
                                                    decimal=5)
                        else:
                            npt.assert_almost_equal(x_defects[:, 0],
                                                    np.array([ 4.91755352e-02, -1.65673365e-02, -3.09782083e-03,  1.22322499e+00,
                                                   -3.17079402e+01,  1.12985229e+02, -3.36444200e+00,  4.02911242e+00,
                                                   -1.03517852e+01,  3.09538706e+00,  1.52954858e-01,  1.01881033e+01]),
                                                    decimal=5)
                    else:
                        if with_external_force:
                            npt.assert_almost_equal(x_defects[:, 0],
                                                    np.array([-0.24539012, -0.38253111, -0.13408166,  0.49788677, 13.85407958,
                                                    9.69500013]),
                                                    decimal=5)
                        else:
                            npt.assert_almost_equal(x_defects[:, 0],
                                                    np.array([ 3.02515888e-01,  1.40184307e-01, -4.72979527e-01,  1.72167922e+00,
                                                    -3.48494533e+01,  1.46223566e+02]),
                                                    decimal=5)
            elif ContactType.RIGID_IMPLICIT in contact_types:
                if with_residual_torque:
                    if with_excitations:
                        if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                            ...
                            npt.assert_almost_equal(x_defects[:, 0],
                                                    np.array([ 6.58474157e-02, -2.21841265e-02, -4.14806864e-03, -1.70266428e+00,
                                               -1.31826147e+01, -1.28447704e+01,  3.25656270e+00, -2.72705314e+00,
                                                4.76521837e-01, -5.19721258e+00,  1.14956080e+01,  8.64588438e+00]),
                                                    decimal=5)
                    else:
                        if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                            ...
                            npt.assert_almost_equal(x_defects[:, 0],
                                                    np.array([  0.17405995,   0.08065849,  -0.2721404 ,  -1.89342602,
                                                        -16.46421687,  -7.88393296]),
                                                    decimal=5)
                else:
                    if with_excitations:
                        if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                            ...
                            npt.assert_almost_equal(x_defects[:, 0],
                                                    np.array([ 4.91755352e-02, -1.65673365e-02, -3.09782083e-03, -2.51010444e+00,
                                               -1.36097225e+01, -1.32621814e+01, -3.36444200e+00,  4.02911242e+00,
                                               -1.03517852e+01,  3.09538706e+00,  1.52954858e-01,  1.01881033e+01]),
                                                    decimal=5)
                    else:
                        if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                            ...
                            npt.assert_almost_equal(x_defects[:, 0],
                                                    np.array([ 0.30251589,  0.14018431, -0.47297953, -2.28210331, -9.70670533,
                                                    -7.06713657]),
                                                    decimal=5)
            else:
                if with_residual_torque:
                    if with_excitations:
                        if with_external_force:
                            if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                                ...
                                npt.assert_almost_equal(x_defects[:, 0],
                                                        np.array([ -0.39414169,  -0.34540175,  -0.64642364,   0.31041669,
                                                           -19.22248632,  -6.93204239, -29.35153822,  -7.14085992,
                                                            18.29768098,  11.45405575,   7.31449586,   4.18729136]),
                                                        decimal=5)
                        else:
                            if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                        ...
                        if with_external_force:
                            if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                                ...
                                npt.assert_almost_equal(x_defects[:, 0],
                                                        np.array([ -0.14206822,  -0.22146577,  -0.07762636,  -2.14326015,
                                                    -19.93936948,  -6.05447738]),
                                                        decimal=5)
                        else:
                            if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                                npt.assert_almost_equal(x_defects[:, 0],
                    ...
                    if with_excitations:
                        if with_external_force:
                            if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                                ...
                                npt.assert_almost_equal(x_defects[:, 0],
                                                        np.array([ -0.06160796,  -0.05398946,  -0.10104194,  -0.59784919,
                                                       -19.46454159,  -7.29982552,  -5.35239171,   1.1691733 ,
                                                         1.95016256,  -0.72025152,   1.05381874,   0.4267984 ]),
                                                        decimal=5)
                        else:
                            if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                                npt.assert_almost_equal(x_defects[:, 0],
                        ...
                        if with_external_force:
                            if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
                                ...
                                npt.assert_almost_equal(x_defects[:, 0],
                                                        np.array([-0.24539012, -0.38253111, -0.13408166, -2.2628544 , -8.36653992,
                                                     -3.06929409]),
                                                        decimal=5)
                        else:
                            if defects_type == DefectType.QDDOT_EQUALS_FORWARD_DYNAMICS:
...
def test_joints_acceleration_driven(cx, phase_dynamics, defects_type):
    ...
    if defects_type == DefectType.TAU_EQUALS_INVERSE_DYNAMICS:
        ...
        npt.assert_almost_equal(x_defects[:, 0],
                                np.array([0.18430491, 0.18757883, 1.05031557, 0.21394574]),
                                decimal=5)
