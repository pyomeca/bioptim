...
def prepare_ocp_multinode_constraint(biorbd_model_path, phase_1, phase_2) -> OptimalControlProgram:
...
def prepare_ocp_multinode_objective(biorbd_model_path, phase_1, phase_2) -> OptimalControlProgram:
    biorbd_model = (biorbd.Model(biorbd_model_path), biorbd.Model(biorbd_model_path), biorbd.Model(biorbd_model_path))
    ...
    multinode_objectives.add(
        MultinodeObjectiveFcn.COM_EQUALITY,
        phase_first_idx=phase_1,
        phase_second_idx=phase_2,
        first_node=Node.START,
        second_node=Node.START,
    ...
    multinode_objectives.add(
        MultinodeObjectiveFcn.COM_VELOCITY_EQUALITY,
        phase_first_idx=phase_1,
        phase_second_idx=phase_2,
        first_node=Node.START,
        second_node=Node.START,
...
def test_multinode_constraint_fail_first_node(node):
    # Constraints
    multinode_constraints = MultinodeConstraintList()
    # hard constraint
    with pytest.raises(
...
@pytest.mark.parametrize("node", [Node.ALL, Node.INTERMEDIATES, Node.ALL_SHOOTING])
def test_multinode_constraint_fail_second_node(node):
    # Constraints
    multinode_constraints = MultinodeConstraintList()
    # hard constraint
    with pytest.raises(
...
def test_multinode_constraint_wrong_phase(phase_1, phase_2):
...
@pytest.mark.parametrize("node", [Node.ALL, Node.INTERMEDIATES, Node.ALL_SHOOTING])
def test_multinode_objective_fail_first_node(node):
    # Objectives
    multinode_objectives = MultinodeObjectiveList()
    # hard objective
    with pytest.raises(
...
@pytest.mark.parametrize("node", [Node.ALL, Node.INTERMEDIATES, Node.ALL_SHOOTING])
def test_multinode_objective_fail_second_node(node):
    # Objectives
    multinode_objectives = MultinodeObjectiveList()
    # hard objective
    with pytest.raises(
...
@pytest.mark.parametrize("phase_1", [-1, 0, 4])
@pytest.mark.parametrize("phase_2", [-1, 0, 4])
def test_multinode_objective_wrong_phase(phase_1, phase_2):
    model = TestUtils.bioptim_folder() + "/examples/getting_started/models/cube.bioMod"
    if phase_1 == 4 or (phase_1 == 0 and phase_2 == 4) or (phase_1 == -1 and phase_2 == 4):
        with pytest.raises(
            RuntimeError,
