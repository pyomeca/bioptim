...
class ObjectiveFunction:
    ...
    def minimize_states(ocp, nlp, weight=1, states_idx=(), data_to_track=()):
        states_idx = ObjectiveFunction.__check_var_size(
            states_idx, nlp["nx"], "state_idx"
        )
    ...
    def minimize_markers(ocp, nlp, weight=1, markers_idx=(), data_to_track=()):
        ...
        markers_idx = ObjectiveFunction.__check_var_size(
            markers_idx, n_mark, "markers_idx"
        )
        data_to_track = ObjectiveFunction.__check_tracking_data_size(
    ...
    def minimize_torque(ocp, nlp, weight=1, controls_idx=(), data_to_track=()):
        n_tau = nlp["nbTau"]
        controls_idx = ObjectiveFunction.__check_var_size(
            controls_idx, n_tau, "controls_idx"
        )
        data_to_track = ObjectiveFunction.__check_tracking_data_size(
            data_to_track, [nlp["ns"], len(controls_idx)]
        )
    ...
    def minimize_muscle(ocp, nlp, weight=1, muscles_idx=(), data_to_track=()):
        ...
        muscles_idx = ObjectiveFunction.__check_var_size(
            muscles_idx, nb_muscle, "muscles_idx"
        )
        data_to_track = ObjectiveFunction.__check_tracking_data_size(
            data_to_track, [nlp["ns"], len(muscles_idx)]
        )
    ...
    def __check_var_size(var_idx, target_size, var_name="var"):
        if var_idx == ():
            ...
            if max(var_idx) > target_size:
                raise RuntimeError(
                    var_name
                    + " in minimize_states cannot be higher than nx ("
                    + target_size
                    + ")"
                )
        return var_idx
